...
function handleIconDoubleClick(icon) {
        ...
        dockIcon.addEventListener("click", function() {
            if (programWindow.classList.contains("minimized")) {
                ...
                if (programWindow.isMaximized) {
                    programWindow.classList.add("maximized"); // Restore maximized state
                }
            } else {
    ...
    function handleWindowClick(event) {
        ...
        if (clickedWindow) {
            ...
            // Add "focused" class to the clicked window
            clickedWindow.classList.add("focused");

            // Move the clicked window to the front of the stack (update z-index)
            const maxZIndex = Array.from(desktop.querySelectorAll(".program-window")).reduce((maxZ, window) => Math.max(maxZ, parseInt(window.style.zIndex || 0)), 0);
    ...
    function handleDesktopMouseDown(event) {
        // Check if the clicked element is an icon or a window
    ...
    function handleDesktopMouseUp() {
        if (isDragging) {
            ...
            // Use a timeout to wait for the animation to finish before hiding
            setTimeout(() => {
                selectionBox.style.display = "none";
                selectionBox.classList.remove("fade-out"); // Clean up the class
    ...
    function openProgramWindow(programData) {
        ...
        programWindow.isMaximized = false; // Track maximized state

        // Create title bar
        const titleBar = document.createElement("div");
        titleBar.classList.add("title-bar");
        titleBar.textContent = programData.name; // Set the title
        ...
        const minimizeButton = createButton("REMOVE", "minimize-button", function() {
            if (programWindow.isMaximized) {
                programWindow.classList.add("minimized");
                programWindow.isMaximized = true; // Keep track that it was maximized
            } else {
                programWindow.classList.add("minimized");
            }
        ...
        const maximizeButton = createButton("EXPAND_CONTENT", "maximize-button", function() {
            ...
            programWindow.classList.remove("minimized");
            programWindow.isMaximized = true; // Update state on maximize
            toggleMaximizeButton();
        });
        const unmaximizeButton = createButton("COLLAPSE_CONTENT", "unmaximize-button", function() {
            programWindow.classList.remove("maximized");
            programWindow.isMaximized = false; // Update state on unmaximize
            toggleMaximizeButton();
        });

        // Function to toggle between maximize and unmaximize buttons
        ...
        // Set the z-index to a high value to ensure it appears on top
        ...
        programWindow.style.zIndex = maxZIndex + 1; // Set new window's z-index to be above existing ones
        ...
        function handleMouseDown(event) {
            if (programWindow.isMaximized) {
                return; // Prevent dragging if the window is maximized
            }
            isDragging = true;
        ...
        function handleMouseMove(event) {
            if (isDragging && !programWindow.isMaximized) { // Only allow dragging if not maximized
        ...
        dockIcon.classList.add("dock-icon");
        dockIcon.style.width = "46px"; // Set the width
        dockIcon.style.height = "46px"; // Set the height
        ...
        dockIcon.addEventListener("click", function() {
            if (programWindow.classList.contains("minimized")) {
                ...
                if (programWindow.isMaximized) {
                    ...
                    // If it was not maximized, just show it
                    programWindow.classList.remove("maximized");
                }
            } else {
                programWindow.classList.add("minimized");
